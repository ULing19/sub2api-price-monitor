...
因此你本机保存过的站点、登录状态、SMTP 通知设置、变化记录、对比基线和历史价格不会进入发行版；其他人打开后会从空配置开始使用。站点密码和 SMTP 密码只保存在当前 Windows 用户的凭据管理器中，也不会进入发行包。
...
```powershell
.\tools\packaging\build_price_app.ps1 -Version 1.2.2
```
