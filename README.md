...
- 变化记录：独立记录倍率、价格、分组信息和抓取状态变化，不把变化事件混入当前价格快照。
...
主界面采用左侧工作区导航，当前价格、站点授权、价格变化和运行日志各自独立。顶部可直接切换 `OpenAI`、`Anthropic`、`Gemini`、`Grok`，统计区展示当前快照中的套餐、分组和模型分类数量。`全部` 页按价格升序展示，模型分类页按倍率升序展示。
...
在 `站点与授权` 页面保存网址、备注、API 路径、更新间隔和 `自动检查此站点` 开关，再点击 `打开 WebView 登录`。你可以在站点窗口里正常登录、完成安全验证或进入控制台；软件不绕过验证，只复用已经登录成功的 WebView Cookie/session。登录态可用后，软件会自动调用 Sub2API 接口抓取当前价格，并把 WebView 窗口收起。
...
`price-changes.json` 保存变化记录及其已读状态，`price-change-baselines.json` 保存各站点用于下次比较的本地基线。两者都可能包含站点地址和价格信息，只保存在当前用户的数据目录中。
...
诊断日志最多保留 4 个约 1 MB 的本地文件，并会隐藏常见 Bearer Token 和 `sk-` 密钥格式。日志、站点配置、Cookie、密码、价格变化、对比基线和价格历史都只保存在当前用户电脑上。

## 隐私边界

发行版只包含程序本体和抓取脚本，不会包含本机的 `output/`、保存站点、价格变化、对比基线、价格历史或 WebView 登录缓存。其他人运行 exe 后会从空配置开始，自己的站点和登录状态会保存到自己的本机目录。
